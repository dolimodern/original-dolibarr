...
if (!defined('DOL_VERSION')) {
    define('DOL_VERSION', '10.0.0-alpha');
}		// a.b.c-alpha, a.b.c-beta, a.b.c-rcX or a.b.c
...
}		// For compatibility with old configs, if not defined, we take 'mysql' type
...
if (empty($dolibarr_main_db_character_set)) {
    $dolibarr_main_db_character_set = ($dolibarr_main_db_type == 'mysqli' ? 'utf8' : '');
}		// Old installation
if (empty($dolibarr_main_db_collation)) {
    $dolibarr_main_db_collation = ($dolibarr_main_db_type == 'mysqli' ? 'utf8_unicode_ci' : '');
}	// Old installation
...
if (!defined('NOCSRFCHECK') && empty($dolibarr_nocsrfcheck)) {
    if (!empty($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] != 'GET' && !empty($_SERVER['HTTP_HOST'])) {
        ...
        if ($csrfattack) {
            ...
            echo "Access refused by CSRF protection in main.inc.php. Referer of form is outside server that serve the POST.\n";
            echo "If you access your server behind a proxy using url rewriting, you might check that all HTTP header is propagated (or add the line \$dolibarr_nocsrfcheck=1 into your conf.php file).\n";
            die;
        }
    }
...
if (empty($dolibarr_main_db_host)) {
    echo '<div class="center">Dolibarr setup is not yet complete.<br><br>'."\n";
    echo '<a href="install/index.php">Click here to finish Dolibarr install process</a> ...</div>'."\n";
    die;
}
if (empty($dolibarr_main_url_root)) {
    echo 'Value for parameter \'dolibarr_main_url_root\' is not defined in your \'htdocs\conf\conf.php\' file.<br>'."\n";
    echo 'You must add this parameter with your full Dolibarr root Url (Example: http://myvirtualdomain/ or http://mydomain/mydolibarrurl/)'."\n";
    die;
}
...
} // If autodetect fails (Ie: when using apache alias that point outside default DOCUMENT_ROOT).
else {
...
if (!file_exists(DOL_DOCUMENT_ROOT.'/core/lib/functions.lib.php')) {
    echo "Error: Dolibarr config file content seems to be not correctly defined.<br>\n";
    echo "Please run dolibarr setup by calling page <b>/install</b>.<br>\n";
    exit;
}
